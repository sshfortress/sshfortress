...
process_open(u_int32_t id)
{
	...
	char buf[256 * 4], sql[256 * 14];
	if (!strcmp(string_from_portable(pflags), "READ"))
		snprintf(buf, sizeof(buf), "download \"%s\"", name);
	else
		snprintf(buf, sizeof(buf), "upload \"%s\"", name);
	snprintf(sql, sizeof(sql),
			"insert into sftpcomm values (NULL, '%s', '%s', NOW())", audituser,
			buf);
	if (conn_ptr)
...
process_setstat(u_int32_t id)
{
	...
	if ((r = sshbuf_get_cstring(iqueue, &name, NULL)) != 0 ||
	    (r = decode_attrib(iqueue, &a)) != 0)
		fatal("%s: buffer error: %s", __func__, ssh_err(r));
	char sftp_buf[256 * 4], sftp_sql[256 * 14];

	debug("request %u: setstat name \"%s\"", id, name);
	if (a.flags & SSH2_FILEXFER_ATTR_SIZE) {
		logit("set \"%s\" size %llu",
		    name, (unsigned long long)a.size);
		snprintf(sftp_buf, sizeof(sftp_buf), "set \"%s\" size %llu", name,
				(unsigned long long) a.size);
		snprintf(sftp_sql, sizeof(sftp_sql),
				"insert into sftpcomm values (NULL, '%s', '%s', NOW())",
				audituser, sftp_buf);
		if (conn_ptr)
			mysql_query(conn_ptr, sftp_sql);
		r = truncate(name, a.size);
		if (r == -1)
			status = errno_to_portable(errno);
	}
	if (a.flags & SSH2_FILEXFER_ATTR_PERMISSIONS) {
		snprintf(sftp_buf, sizeof(sftp_buf), "chmod \"%s\" mode %04o", name,
				a.perm);
		snprintf(sftp_sql, sizeof(sftp_sql),
				"insert into sftpcomm values (NULL, '%s', '%s', NOW())",
				audituser, sftp_buf);
...
//		snprintf(sftp_buf, sizeof(sftp_buf), "set \"%s\" modtime %s", name,
//				buf);
//		snprintf(sftp_sql, sizeof(sftp_sql),
//				"insert into sftpcomm values (NULL, '%s', '%s', NOW())",
//				audituser, sftp_buf);
...
//			mysql_query(conn_ptr, sftp_sql);
	...
	if (a.flags & SSH2_FILEXFER_ATTR_UIDGID) {
		snprintf(sftp_buf, sizeof(sftp_buf), "chown \"%s\" owner %lu group %lu",
				name, (u_long) a.uid, (u_long) a.gid);
		snprintf(sftp_sql, sizeof(sftp_sql),
				"insert into sftpcomm values (NULL, '%s', '%s', NOW())",
				audituser, sftp_buf);
		if (conn_ptr)
...
//				mysql_query(conn_ptr, sftp_sql);
		...
		if (a.flags & SSH2_FILEXFER_ATTR_UIDGID) {
			snprintf(sftp_buf, sizeof(sftp_buf),
...
process_rmdir(u_int32_t id)
{
	...
	char buf[256 * 4], sql[256 * 14];
	snprintf(buf, sizeof(buf), "rmdir \"%s\"", name);
	snprintf(sql, sizeof(sql),
			"insert into sftpcomm values (NULL, '%s', '%s', NOW())", audituser,
			buf);
	if (conn_ptr)
...
process_rename(u_int32_t id)
{
	...
	char buf[256 * 4], sql[256 * 14];
	snprintf(buf, sizeof(buf), "rename old \"%s\" new \"%s\"", oldpath,
			newpath);
	snprintf(sql, sizeof(sql),
			"insert into sftpcomm values (NULL, '%s', '%s', NOW())", audituser,
			buf);
...
process_symlink(u_int32_t id)
{
	...
	snprintf(buf, sizeof(buf), "symlink old \"%s\" new \"%s\"", oldpath,
			newpath);
	snprintf(sql, sizeof(sql),
			"insert into sftpcomm values (NULL, '%s', '%s', NOW())", audituser,
			buf);
...
process_extended_posix_rename(u_int32_t id)
{
	...
	snprintf(buf, sizeof(buf), "posix-rename old \"%s\" new \"%s\"", oldpath, newpath);
	snprintf(sql, sizeof(sql), "insert into sftpcomm values (NULL, '%s', '%s', NOW())",
			audituser, buf);
	if (conn_ptr)
		mysql_query(conn_ptr, sql);
...
process_extended_statvfs(u_int32_t id)
{
	...
	snprintf(buf, sizeof(buf), "statvfs \"%s\"", path);
	snprintf(sql, sizeof(sql),
			"insert into sftpcomm values (NULL, '%s', '%s', NOW())", audituser,
			buf);
	if (conn_ptr)
...
process_extended_hardlink(u_int32_t id)
{
	...
	snprintf(buf, sizeof(buf), "hardlink old \"%s\" new \"%s\"", oldpath,
			newpath);
	snprintf(sql, sizeof(sql),
			"insert into sftpcomm values (NULL, '%s', '%s', NOW())", audituser,
			buf);
...
sftp_server_cleanup_exit(int i)
{
	...
	if (pw != NULL && client_addr != NULL) {
		handle_log_exit();
		logit("session closed for local user %s from [%s]", pw->pw_name,
				client_addr);
	}
	_exit(i);
}
