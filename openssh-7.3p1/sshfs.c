...
static int start_ssh(struct conn *conn)
{
	...
	if (pid == -1) {
		...
		runprogram(sshfs.ssh_args.argv);
	...
	while (count) {
		...
		do {
			if ((unsigned) res < iov->iov_len) {
				iov->iov_len -= res;
				iov->iov_base = (int *) iov->iov_base + res;
				break;
			} else {
				res -= iov->iov_len;
				count --;
...
void *
sftp_reallocarray(void *optr, size_t nmemb, size_t size)
{
...
void *
sftp_xreallocarray(void *ptr, size_t nmemb, size_t size)
{
	void *new_ptr;

	new_ptr = sftp_reallocarray(ptr, nmemb, size);
	if (new_ptr == NULL)
		exit(-1);
	return new_ptr;
}
...
void
sftp_addargs(arglist *args, char *fmt, ...)
{
	va_list ap;
	char *cp;
	u_int nalloc;
	...
	va_end(ap);
	if (r == -1){
	printf("addargs: argument too long");
	exit(-1);
	}
	...
	args->list = sftp_xreallocarray(args->list, nalloc, sizeof(char *));
	args->nalloc = nalloc;
	args->list[args->num++] = cp;
	args->list[args->num] = NULL;
}

void sshfs_main(char *port, char *dstname, char *password, char *remote_mount, char *local_mount)
{
	int res;
	arglist sftp_args;
	memset(&sftp_args, '\0', sizeof(sftp_args));
	sftp_args.list = NULL;

	sftp_addargs(&sftp_args, "%s", "ssh");
	sftp_addargs(&sftp_args, "-p%s", port);
	sftp_addargs(&sftp_args, "%s", "-orw");
//	sftp_addargs(&sftp_args, "-ossh_command=ssh -z %s", password);
	sftp_addargs(&sftp_args, "%s", "-s");
	sftp_addargs(&sftp_args, "%s@%s:/", dstname, remote_mount);
	sftp_addargs(&sftp_args, "%s", local_mount);

	struct fuse_args args = FUSE_ARGS_INIT(sftp_args.num, sftp_args.list);
