...
initialize_server_options(ServerOptions *options)
{
	...
	options->sshfortress_host = NULL;
	options->sshfortress_user = NULL;
	options->sshfortress_password = NULL;
	options->sshfortress_database = NULL;
	options->sshfortress_port = NULL;
}
...
fill_default_server_options(ServerOptions *options)
{
	...
	if (options->sshfortress_port == NULL)
		options->sshfortress_port = xstrdup("3306");
	if (options->sshfortress_host == NULL)
		options->sshfortress_host = xstrdup("127.0.0.1");
	if (options->sshfortress_user == NULL)
		options->sshfortress_user = xstrdup("audit");
	if (options->sshfortress_password == NULL)
		options->sshfortress_password = xstrdup("audit");
	if (options->sshfortress_database == NULL)
		options->sshfortress_database = xstrdup("audit_sec");
...
#define CLEAR_ON_NONE(v) \
	...
	CLEAR_ON_NONE(options->sshfortress_host);
	CLEAR_ON_NONE(options->sshfortress_user);
	CLEAR_ON_NONE(options->sshfortress_password);
	CLEAR_ON_NONE(options->sshfortress_database);
	CLEAR_ON_NONE(options->sshfortress_port);
...
typedef enum {
	...
	sDeprecated, sUnsupported, sSSHfortress_host,  sSSHfortress_user,
	sSSHfortress_password, sSSHfortress_database, sSSHfortress_port
} ServerOpCodes;
...
static struct {
	...
	{ "sshfortress_host", sSSHfortress_host, SSHCFG_GLOBAL },
	{ "sshfortress_user", sSSHfortress_user, SSHCFG_GLOBAL },
	{ "sshfortress_password", sSSHfortress_password, SSHCFG_GLOBAL },
	{ "sshfortress_database", sSSHfortress_database, SSHCFG_GLOBAL },
	{ "sshfortress_port", sSSHfortress_port, SSHCFG_GLOBAL },
	{ NULL, sBadOption, 0 }
};
    ...
    struct connection_info *connectinfo)
{
 ...
 parse_keytypes:
	...
	case sSSHfortress_host:
		charptr = &options->sshfortress_host;
		arg = strdelim(&cp);
		if (!arg || *arg == '\0')
	...
	case sSSHfortress_user:
		charptr = &options->sshfortress_user;
		arg = strdelim(&cp);
		if (!arg || *arg == '\0')
			fatal("%s line %d: missing file name.", filename, linenum);
		if (*activep && *charptr == NULL)
	...
	case sSSHfortress_password:
		charptr = &options->sshfortress_password;
		arg = strdelim(&cp);
		if (!arg || *arg == '\0')
			fatal("%s line %d: missing file name.", filename, linenum);
		if (*activep && *charptr == NULL)
	...
	case sSSHfortress_database:
		charptr = &options->sshfortress_database;
		arg = strdelim(&cp);
		if (!arg || *arg == '\0')
			fatal("%s line %d: missing file name.", filename, linenum);
		if (*activep && *charptr == NULL)
	...
	case sSSHfortress_port:
		charptr = &options->sshfortress_port;
		arg = strdelim(&cp);
		if (!arg || *arg == '\0')
			fatal("%s line %d: missing file name.", filename, linenum);
		if (*activep && *charptr == NULL)
